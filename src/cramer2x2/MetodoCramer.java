...
public class MetodoCramer {
    ...
    public void rellenarMatrizConnumerosAleatorios(int min, int max) {
        Random random = new Random();
        for (int i = 0; i < filas; i++) {
            for (int j = 0; j < columnas; j++) {
                matriz[i][j].setNumerador(random.nextInt(max - min + 1) + min);
                matriz[i][j].setDenominador(1);
    ...
    // yo le puse delta S pero se trata de la determinante de lq primer matriz aqui
